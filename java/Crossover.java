...
public class Crossover {
	...
	public static Genetic_algorithm best_cost_route_crossover(Genetic_algorithm ga, Load_problem problem, int gen_num, Particle_swarm para) {
		...
		for (int i=0; i<ga.pool.size()/2; i++) {
			d = Math.random();
			count = count + 1;
			if (gen_num == 1 && count == 1) { //determine pc and pm at random within predetermined value
				for (int j=0; j<ga.pool.size(); j++) {
					List<Double> para_temp = new ArrayList<Double>();
					para_temp.add(Math.random()); //add pc (0.0-1.0)
					para_temp.add(Math.random()); //add pm (0.0-1.0)
					para.lb_para.add(para_temp);
					List<Double> para_temp_ = new ArrayList<Double>(para_temp);
					para.cr_para.add(para_temp_);
				}
				for (int j=0; j<ga.pool.size()/2; j++) {
					//crossover probability of former half and latter half have to be same
					para.lb_para.get(j).set(0, para.lb_para.get(ga.pool.size()/2+j).get(0));
					para.cr_para.get(j).set(0, para.cr_para.get(ga.pool.size()/2+j).get(0));
				}
			}
			
			if (d<para.cr_para.get(i).get(0)) {  //Crossover occers
				...
				para.cr_para.get(worst_ind).set(0, para.cr_para.get(i).get(0)); //get pc
	...
	public static Genetic_algorithm Uniform_order_crossover(Genetic_algorithm ga, Load_problem problem, int gen_num, Particle_swarm para) {
		long seed = Runtime.getRuntime().freeMemory();
		Random rnd = new Random(seed);
		double d;
		int count = 0;
