...
public class Genetic_algorithm {
	...
	public static Genetic_algorithm ga(Load_problem problem, Genetic_algorithm ga, long start) {
		...
		Particle_swarm para = new Particle_swarm();
		...
		if(problem.evaluation == 2) ga = Evaluate.pareto_ranking(problem, ga, gen_num, para);
		...
		for (gen_num=1; gen_num<problem.maxgen; gen_num++) {
			...
			ga = Tournament.k_tournament(ga, problem, gen_num, para);
			ga = Elitism.elitism(ga, problem, x, para, gen_num);
			if(problem.crossover == 0) ga = Crossover.best_cost_route_crossover(ga, problem, gen_num, para);
			if(problem.crossover == 1) ga = Crossover.Uniform_order_crossover(ga, problem, gen_num, para);
			ga = Mutation.constrained_route_reversal_mutation(ga, problem, para);
			ga = Elitism.elitism(ga, problem, x, para, gen_num);
			...
			*/
			...
			//assign the number to each solution
			for (int i=0; i<ga.pool.size(); i++) {
				ga.pop.get(i).popnum = i;
			}
			...
			case 1:
				for (int i=0; i<ga.pop.size(); i++) {
					para.min_vehinum_pop.add(ga.pop.get(i).n);
					para.min_distance_pop.add(ga.pop.get(i).J);
				}
				break;
			default:
				for (int i=0; i<ga.pop.size(); i++) {
					if (para.min_vehinum_pop.get(i) >= ga.pop.get(i).n && para.min_distance_pop.get(i) > ga.pop.get(i).J) { 
						para.min_vehinum_pop.set(i, ga.pop.get(i).n);
						para.min_distance_pop.set(i, ga.pop.get(i).J);
						//update local best parameter
						para.lb_para.get(i).set(0, para.cr_para.get(i).get(0)); //get pc
						para.lb_para.get(i).set(1, para.cr_para.get(i).get(1)); //get pm
					}
				}	
			}
			...
			para = Particle_swarm.parameter_update(problem, para);
		}
		return ga;
	}
}
